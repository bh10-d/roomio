...
function ensureStringId(entityName: string, id: unknown): asserts id is string {
  if (typeof id !== 'string' || id.trim().length === 0) {
    throw new Error(
      `Seed expected ${entityName}.id to be string UUID, received: ${String(id)}`,
    );
  }
}
...
async function seed(): Promise<void> {
  ...
  const userSessionRepo = dataSource.getRepository(UserSession);
  ...
  const sessionUsers = users
    ...
    .slice(0, Math.min(6, users.length));
  for (let i = 0; i < sessionUsers.length; i += 1) {
    ...
    const session = await userSessionRepo.save(
      userSessionRepo.create({
        ...
        device_id: `seed-device-${i + 1}`,
        user_agent: `SeedAgent/${i + 1}`,
        ip_address: `10.10.0.${i + 10}`,
  ...
  const occupiedRoomCount = Math.max(
    1,
    Math.floor(rooms.length * OCCUPIED_ROOM_RATE),
  );
  ...
  for (let i = 0; i < rooms.length; i += 1) {
    rooms[i].status = occupiedRoomIds.has(rooms[i].id)
      ? 'occupied'
      : 'available';
  }
  ...
  for (let i = 0; i < occupiedRooms.length; i += 1) {
    ...
    for (
      let monthOffset = 0;
      monthOffset < BILL_MONTHS_PER_OCCUPIED_ROOM;
      monthOffset += 1
    ) {
      ...
      await paymentRepo.save(
        paymentRepo.create({
          bill_id: bill.id,
          amount:
            billStatus === 'paid' ? totalAmount : Math.floor(totalAmount * 0.6),
  ...
  console.log(
    `Seed success: users=${users.length}, userSessions=${userSessions.length}, houses=${houses.length}, floors=${floors.length}, rooms=${rooms.length}, roomUsers=${roomUsers.length}, contracts=${contracts.length}, bills=${bills.length}, meterReadings=${meterReadings.length}, maintenance=${maintenanceRequests.length}, notifications=${notifications.length}`,
  );
